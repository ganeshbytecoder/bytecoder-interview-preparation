        ...
        for (int i = 0; i < graph.getMatrix().length; i++) {
            for (int j = 0; j < graph.getMatrix()[0].length; j++) {
                ...
                if (graph.getMatrix()[i][j] != -1) {
                    edges.add(new Edge<>(i, j, graph.getMatrix()[i][j]));
                }
            }
        }
...
//        Detect Cycle in Undirected GraphProblems using BFS/DFS Algorithm
    ...
    public void printPrimMST() {
        ...
        Comparator<Edge> comparator = (edge1, edge2) -> edge1.getCost() - edge2.getCost();
        PriorityQueue<Edge> priorityQueue = new PriorityQueue<>(comparator);

//         pick start node
        priorityQueue.add(new Edge<>(0, 0, 0));

        List<Edge> ansEdges = new ArrayList<>();
    ...
    @Override
    public List<Edge> printKrushkalMST() {
    ...
    public List<Edge> printKrushkalMST_M2() {

        PriorityQueue<Edge> edges = new PriorityQueue<>((e1, e2) -> e1.getCost() - e2.getCost());
    ...
    private Edge getEdgeBetween(int start, int end) {
