* https://github.com/donnemartin/system-design-primer
* https://github.com/iluwatar/java-design-patterns/tree/master

Amazon

* https://www.amazon.jobs/en/software-development-interview-prep?%20INTCMPID=OAAJAZ100026B%23/lessons/i42fEs1z6xTY6sORn8uAxp5Wz7iXE5Ck%20#/lessons/fxggI6Y3AxoOjvF9oKV_gky-TSFACjCu
* https://www.linkedin.com/pulse/how-interview-amazon-leadership-david-anderson/
* https://www.techinterviewhandbook.org/grind75/
* https://www.amazon.jobs/en/software-development-interview-prep?cmpid=ECOTOT700005B#/lessons/r2ZOm8qwBm3sSRv2xd8Q7K3BA2qIzjPX
* https://www.amazon.jobs/en/software-development-interview-prep?cmpid=ECOTOT700005B#/lessons/jmmGonbjV2OJp6iV-UMEoqPB6_3_Y33O
* https://www.youtube.com/watch?v=gNQ9-kgyHfo
* https://www.interviewhelp.io/blog/posts/amazon-system-design-interview-questions/
* https://github.com/donnemartin/system-design-primer/tree/master/solutions/system_design/pastebin
* https://www.youtube.com/channel/UC9vLsnF6QPYuH51njmIooCQ
* https://www.educative.io/
* https://www.amazon.com/System-Design-Interview-insiders-Second/dp/B08CMF2CQF/ref=tmm_pap_swatch_0?_encoding=UTF8&sr=
* https://www.amazon.com/dp/1736049119/ref=tsm_1_fb_lk
* https://www.amazon.com/Cracking-Coding-Interview-Programming-Questions/dp/0984782850/ref=sr_1_3?dchild=1&keywords=cracking+coding+interview&qid=1623709878&sr=8-3
* https://www.pramp.com/#/
* https://www.digitalocean.com/community/conceptual-articles/s-o-l-i-d-the-first-five-principles-of-object-oriented-design
* https://www.interviewcake.com/
* https://interviewing.io/

# **Meta  interview Preps**


Your notes provide a solid foundation but need Meta-specific enhancements and deeper coverage in certain areas.

## Meta Interview Preparation Checklist

### Core System Design Topics

* [ ] Scalability (Horizontal vs Vertical)
* [ ] Load Balancing
* [ ] Caching Strategies
* [ ] Database Design (SQL vs NoSQL)
* [ ] API Design (REST, GraphQL)
* [ ] Microservices Architecture
* [ ] CAP Theorem
* [ ] Consistent Hashing (needs more depth)
* [ ] Rate Limiting (needs more implementation details)
* [ ] Data Partitioning/Sharding (needs more examples)
* [ ] Message Queues and Event-Driven Architecture
* [ ] CDN Architecture

### Meta-Specific Systems

* [ ] News Feed Design
* [ ] Messenger/WhatsApp Architecture
* [ ] Instagram Stories/Reels
* [ ] Facebook Search
* [ ] Real-time Notifications System
* [ ] Social Graph Storage and Traversal
* [ ] Content Moderation Systems
* [ ] Privacy Controls Architecture

### Quantitative Design Skills

* [ ] Back-of-the-envelope Calculations
* [ ] Capacity Estimation
* [ ] Traffic Estimation
* [ ] Storage Requirements Calculation
* [ ] Bandwidth Estimation
* [ ] Latency Budgeting

### Low-Level Design

* [X] SOLID Principles
* [X] Design Patterns (Creational, Structural, Behavioral)
* [X] OOP Concepts
* [ ] Concurrency Patterns
* [ ] API Design Best Practices

## Preparation Plan

### Week 1: Foundation Strengthening

* Review existing notes on scalability, caching, load balancing
* Enhance database knowledge with focus on Meta-scale challenges
* Practice capacity estimation for large-scale systems

### Week 2: Meta-Specific Systems

* Study Facebook News Feed design in depth
* Understand real-time messaging systems (Messenger)
* Research Instagram's media storage and delivery architecture
* Focus on social graph implementation

### Week 3: Advanced Distributed Systems

* Deep dive into consensus algorithms
* Study real-time data processing systems
* Understand Meta's approach to global distribution
* Research content delivery optimization

### Week 4: Practice and Mock Interviews

* Solve 5-7 system design questions with time constraints
* Focus on clear communication and structured approach
* Practice drawing clear architecture diagrams
* Get feedback from peers if possible

### Week 5: Final Review

* Review all weak areas identified during practice
* Refresh knowledge on Meta's tech stack and recent innovations
* Prepare concise explanations for common design decisions
* Review quantitative estimation techniques
